...
vdot_locations = strfind(toolboxDir, 'v_');
version = toolboxDir(vdot_locations(end):end);
if ~strcmp(version, 'v_1_1_0b3')
...
outDir   = fullfile(toolboxDir, 'extrapolations', 'auxiliary_functions', 'partition_R');
...
outDir   = fullfile(toolboxDir, 'extrapolations', 'auxiliary_functions', 'shuffle_R_across_trials');
...
outDir   = fullfile(toolboxDir, 'methods', 'auxiliary_functions', 'shuffle_R_across_cells');
...
outDir = fullfile(toolboxDir, 'methods', 'direct_method');
file1Path = fullfile(outDir, 'direct_method_v6a.c');
file2Path = fullfile(toolboxDir, 'bias_corrections', 'bias_corrections_for_direct_method', 'panzeri_and_treves_96.c');

disp(['   Compiling: ' file1Path]);
disp(['   Compiling: ' file2Path]);
