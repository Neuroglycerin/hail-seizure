...
function [featM, bandEdges] = feat_coher_logf(subj, typ)
    ...
    for iChn1=1:nChn
        for iChn2=(iChn1+1):nChn
            ...
            Pxy = pwelch(Dat.data(iChn1,:), Dat.data(iChn2,:), ...
                wndw, overlap, Nfft, Dat.fs, 'cross');
            crspwr = nan(1,nBnd);
            ...
            featM(iSeg,(paircount-1)*nBnd+(1:nBnd)) = abs(crspwr).^2 ./ (bndpwr(iChn1,:).*bndpwr(iChn2,:));
        end
    end
end
