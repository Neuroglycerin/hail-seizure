...
% and normalised against the broadband power
...
%                             estimates in decibels, relative to broadband
...
function [featV, outparams] = feat_psd_logfBB(Dat, inparams)
...
for iChn=1:nChn
    ...
    % Measure broadband power
    BBpwr = sum(Pxx(x(1):end));
    % Sum over each band
    for iBnd=1:nBnd
        % Normalise against broadband power
        bndpwr = sum(Pxx(x(iBnd):x(iBnd+1)-1)) / BBpwr;
