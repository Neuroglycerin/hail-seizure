function [R,sig,ci1,ci2,nan_sig] = corrcoef2(X,Y,varargin)
% CORRCOEF2 calculates the correlation matrix from pairwise correlations.
...
%
% [...] = CORRCOEF2(..., param1, value1, param2, value2, ... );
%	param 		value
...
%       [nan_R,nan_sig]=corrcoef2(X,isnan(X))
%   or  [nan_R,nan_sig]=corrcoef2([X,Y],isnan([X,Y]))
%   or  [R,p,ci1,ci2] = CORRCOEF2(...);
%
...
elseif strcmpi(Mode(1:4),'rank');
	...
	if ~YESNAN,
                ...
                for k = 1:length(jx),
                        ...
                        R(jxo(k),jyo(k)) = corrcoef2(il(:,1),il(:,2));
                end;
		X = ranks(X);
	end;
        
elseif strcmpi(Mode(1:8),'spearman');
        % see [ 6] http://mathworld.wolfram.com/SpearmanRankCorrelationCoefficient.html
        if ~isempty(Y),
                X = [X,Y];
        end;  
        ...
        if ~YESNAN,
                ...
                for k = 1:length(jx),
                        xxx = (iy(:,jx(k)) - iy(:,jy(k))).^2;
                        R(jxo(k),jyo(k)) = sum(xxx);
                        n(jxo(k),jyo(k)) = numel(xxx); % NN is the number of non-missing values
                end;
        else
                for k = 1:length(jx),
                        %ik = ~any(isnan(X(:,[jx(k),jy(k)])),2);
...
elseif strcmpi(Mode(1:7),'partial');
        fprintf(2,'Error CORRCOEF: use PARTCORRCOEF \n',Mode);
        
        return;
        
elseif strcmpi(Mode(1:7),'kendall');
...
[nan_R, nan_sig] = corrcoef2(X,double(isnan(X)));
