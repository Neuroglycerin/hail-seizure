...
% Notch reject
Dat.data = butterfiltfilt(Dat.data', [55 65], Dat.fs, order, 'stop', 'both')';

% Remove 180Hz line noise artifact harmonic (present in Patient_1)
Dat.data = butterfiltfilt(Dat.data', [175 185], Dat.fs, order, 'stop', 'both')';

% Remove 240Hz line noise artifact harmonic (present in Patient_1)
Dat.data = butterfiltfilt(Dat.data', [235 245], Dat.fs, order, 'stop', 'both')';

% Remove <1Hz (present in Patient_2)
% High-pass filter
Dat.data = butterfiltfilt(Dat.data', [0.1 Inf], Dat.fs, order)';
